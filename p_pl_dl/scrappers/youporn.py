...
def run(sUrl, sCookieSource=None, nVideoLimit=None, bDebug=False):
    ...
    def ytdlLoop(lUrls, bLogFailures):
        ...
        for nIdx, sVideoUrl in enumerate(lUrls):
            ...
            dYdlOptions[
                "outtmpl"
            ] = rf".\\sites\\{sExtractor}\\{sVideoId}_%(title).125s.mp4"
...
class Page_Youporn(dl_common.Page):
    ...
    def _get_url_type(self):
        if "youporn.com/watch/" in self.url:
            ...
            "youporn.com/favorites/" in self.url
            or "youporn.com/collections/" in self.url
        ):
            sUrlType = "playlist"
        else:
            raise ValueError(
    ...
    def _extract_page_urls(self, nPage, sFilter=None):
        ...
        lVideos = []
        lTags = soup.find_all(
            attrs={"class": "pcVideoListItem js-pop videoblock videoBox"}
        )
        for tag in lTags:
