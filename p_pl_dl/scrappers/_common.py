...
def cookieHeaderStringGet(dCookies=None):
    ...
    cookieString = "; ".join(f"{key}={value}" for key, value in dCookies.items())
    return cookieString
...
def urlStandardize(sUrl):
    """
    Make sure URL ends with '/' and add f-string brackets for iterating through pages.
    """
    return sUrl.rstrip("/") + "/{}"


class Page:
    def __init__(self, url, headers=dHeaders):
        self.url = url
    ...
    def _extract_video_urls(self, sFilter=None):
        ...
        for a in self.soup.find_all("a", href=True):
            href = a["href"]
            if "http" not in href or (sFilter is not None and sFilter not in href):
                continue
            self.videos.append(href)

    def _html_to_text(self, sFileName="html_content.txt"):
        with open(sFileName, "w", encoding="utf-8") as text_file:
