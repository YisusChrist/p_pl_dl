...
from . import _common as dl_common
...
def run(sUrl, sCookieSource=None, nVideoLimit=None, bDebug=False):
    ...
    html = dl_common.session.get(
        sUrl, headers=dl_common.dHeaders, cookies=dl_common.dCookiesParsed
    )
    ...
    dl_common.sleepRandom(3, 5)

    dYdlOptions = dict(dl_common.dYdlOptions)
    dYdlOptions[
        "download_archive"
    ] = rf".\\sites\\{sExtractor}\\{dYdlOptions['download_archive'].format(sExtractor)}"
    # dYdlOptions['referer'] = 'https://spankbang.com'
    # Not needed - YTDL already has a UA randomizer
    # dYdlOptions['user_agent'] = dl_common.dHeaders['User-Agent']
...
class Page_Spankbang(dl_common.Page):
    def __init__(self, url):
        ...
        self._playlistId = (
            self.url.split(".com/")[1].split("/")[0]
            if self.sUrlType == "playlist"
            else None
        )
        self._sUrlBaseFormat = dl_common.urlStandardize(self.url)
    ...
    def _get_url_type(self):
        ...
        if "/video/" in self.url:
            ...
            raise ValueError(
                f"Unable to determine {sExtractor} URL type for {self.url}! Please submit a bug report!"
            )
        return sUrlType
    ...
    def _extract_page_urls(self, nPage, sFilter=None):
        ...
        for nAttempts in range(3):
            ...
            content = dl_common.session.get(
                sUrlPage, headers=dl_common.dHeaders, cookies=dl_common.dCookiesParsed
            )
    ...
    def _unmask_video_url(self, sUrlMasked, nAttempts=3):
        ...
        for nAttempt in range(nAttempts):
            content = dl_common.session.get(
                sUrlFull, headers=dl_common.dHeaders, cookies=dl_common.dCookiesParsed
            )
            soup = dl_common.BeautifulSoup(content.text, "html.parser")
